...
def syntax_error(insl):
    ...
    for i in range(len(insl)):
        if ins_l[i][0][-1]==":":
            continue
        if ins_l[i][0]=='mov':
            a=check_mov(ins_l[i][1:])
            if (a==-1):
                return -1,linelist[i]
